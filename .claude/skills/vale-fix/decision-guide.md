...
```
Does the file have a `.Procedure` section with numbered steps (`. Step`)?
├── YES → PROCEDURE
│   └── Does it also have `==` subsections?
│       ├── YES → PROCEDURE (but TaskSection will fire separately)
│       └── NO → PROCEDURE ✓
├── NO → Continue checking
│
Does the file have `include::` directives and `assembly_` prefix?
├── YES → ASSEMBLY
└── NO → Continue checking
│
Does the file contain data tables, parameter lists, or lookup info?
├── YES → REFERENCE
└── NO → Continue checking
│
Does the file explain what/why without actionable steps?
├── YES → CONCEPT
└── NO → Continue checking
│
Is the file a reusable fragment (`snip_` prefix, in snippets/)?
├── YES → SNIPPET
└── NO → → MANUAL REVIEW (content type unclear)
```

**Auto-fix when**: Classification is unambiguous (one clear match).
**Manual review when**: Content mixes types, or none of the checks match.
...
Then re-run vale to catch any cascading issues.
...
When a vale violation occurs ADJACENT to these elements, fix only the flagged
text — never modify the protected element.
